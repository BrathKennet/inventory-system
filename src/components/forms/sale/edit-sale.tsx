...
import { editSale } from "@/services/sale/client";
...
export default function EditSaleForm({
  ...
  lots: any[] | null;
  sale: any;
  listSelectInitial: any[];
  detail: any[];
}) {
  const [formState, formAction] = useFormState(editSale, undefined);
  const [listSelect, setListSelect] = useState<any[]>(listSelectInitial);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < listSelectInitial.length; i++) {
      if (listSelectInitial[i] != undefined) {
        const indexSelect = updateLots?.findIndex(
          (v) => v.id === listSelectInitial[i].id
        );
            ...
            <div className="w-[98%] grid sm:grid-cols-2 grid-cols-1 gap-6 mt-4 ">
              ...
              {listLot.map((v, i) => (
                <SelectLot
                  ...
                  defaultValueQ={
                    detail[i] != undefined ? detail[i].quantity : undefined
                  }
                  /* defaultValueQ={detail[i] != undefined ? detail[i].quantity : undefined} */
                />
              ))}
              <button
                type="button"
